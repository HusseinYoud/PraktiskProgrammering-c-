...
)
{
    if (b < a) {
        throw std::invalid_argument(
            "driver: b must be greater than or equal to a"
        );
    }

    if (h <= 0.0) {
        throw std::invalid_argument(
            "driver: h must be positive"
        );
    }

    if (hmax <= 0.0) {
        throw std::invalid_argument(
            "driver: hmax must be positive"
        );
    }

    if (acc < 0.0 || eps < 0.0) {
        throw std::invalid_argument(
            "driver: acc and eps must be non-negative"
        );
    }

    if (acc == 0.0 && eps == 0.0) {
        throw std::invalid_argument(
            "driver: acc and eps cannot both be zero"
        );
    }
